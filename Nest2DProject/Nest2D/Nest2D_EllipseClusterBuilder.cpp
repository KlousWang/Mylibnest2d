    ...
    namespace NEST2DMANAGERLIB {
        namespace {
            ...
            std::vector<std::vector<int>> GroupEllipseIndices(const std::vector<int> &AIndices, const std::vector<TetShapeFeature> &AFeatures)
            {
                ...
                std::sort(Infos.begin(), Infos.end(), [](const TetEllipseIndexInfo &A, const TetEllipseIndexInfo &AB) {
                ...
                Infos.erase(std::unique(Infos.begin(), Infos.end(), [](const TetEllipseIndexInfo &A, const TetEllipseIndexInfo &AB) { return A.Index == AB.Index; }), Infos.end());
        ...
        void CetEllipseClusterBuilder::BuildCandidates(const CetTNestItemVector &AItems, const std::vector<TetShapeFeature> &AFeatures, const std::vector<int> &AIndices, const TetNestOptions &AOptions, std::vector<TetClusterCandidate> &AOut)
        {
            ...
            for (std::size_t GroupIndex = 0; GroupIndex < Groups.size(); ++GroupIndex) {
                ...
                for (std::size_t LargerIndex = GroupIndex + 1; LargerIndex < Groups.size(); ++LargerIndex) {
                    ...
                    if (!LargerGroup.empty() && GroupSize > 0.0 && AFeatures[LargerGroup.front()].EllipseMajorAxis >= GroupSize * CET_ELLIPSE_FILLER_MIN_SIZE_RATIO) {
                        HasLargerFamily = true;
                        break;
                    }
                }
                if (HasLargerFamily && ClusterIndices.size() >= CET_ELLIPSE_FILLER_SINGLE_RESERVE_MIN * 2) {
                    const std::size_t ReserveCount = std::min(CET_ELLIPSE_FILLER_SINGLE_RESERVE_MAX, std::max(CET_ELLIPSE_FILLER_SINGLE_RESERVE_MIN, ClusterIndices.size() / 4));
                    if (ClusterIndices.size() > ReserveCount)
                        ClusterIndices.resize(ClusterIndices.size() - ReserveCount);
                }
        ...
        void CetEllipseClusterBuilder::_BuildSameSizeClusterCandidates(const CetTNestItemVector &AItems, const std::vector<TetShapeFeature> &AFeatures, const std::vector<int> &AIndices, const TetNestOptions &AOptions, std::vector<TetClusterCandidate> &AOut)
        {
            ...
            while (Remaining.size() >= 2) {
                ...
                if (BestCount >= 4) {
                    ...
                    std::vector<int> AlternateIndices(Remaining.begin(), Remaining.begin() + static_cast<std::vector<int>::difference_type>(AlternateCount));
