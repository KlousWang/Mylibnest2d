	...
	namespace NEST2DMANAGERLIB {
		...
		namespace {
			TetCircleFitResult FitCircleCenterFromThreePoints(const ClipperLib::IntPoint& AFirstPoint, const ClipperLib::IntPoint& AMiddlePoint, const ClipperLib::IntPoint& ALastPoint)
			{
				...
				if (std::abs(Denominator) <= CET_SHAPE_EPSILON) return FitResult;
			...
			{
				if (AChain.size() < 3) return TetCircleFitResult{};
			...
			{
				CetPath Chain;
				if (AContour.empty()) return Chain;
			...
			bool EvaluateArcChain(const CetPath& AChain, double ACenterX, double ACenterY, double& AOutAverageRadius, double& AOutAverageError, double& AOutMaxError)
			{
				...
				if (AOutAverageRadius <= CET_SHAPE_EPSILON) return false;
			...
			void EvaluateArcChainPair(const CetPath& AContour, std::size_t AFirstEdgeIndex, std::size_t ASecondEdgeIndex, TetArcChainFitResult& ABestFitResult, CetPath& ABestOuterChain)
			{
				...
				if (FirstChain.size() < 3 || SecondChain.size() < 3) return;
				...
				if (!FirstCenterFit.Valid || !SecondCenterFit.Valid) return;
				...
				if (!EvaluateArcChain(FirstChain, CenterX, CenterY, FirstRadius, FirstAverageError, FirstMaxError) || !EvaluateArcChain(SecondChain, CenterX, CenterY, SecondRadius, SecondAverageError, SecondMaxError)) return;
				const double OuterRadius = std::max(FirstRadius, SecondRadius);
				const double InnerRadius = std::min(FirstRadius, SecondRadius);
				if (InnerRadius <= CET_SHAPE_EPSILON || OuterRadius <= InnerRadius || CenterDistance > OuterRadius * 0.10) return;
				const double Thickness = OuterRadius - InnerRadius;
				if (Thickness < OuterRadius * 0.02 || Thickness > OuterRadius * 0.80) return;
				...
				if (AverageError > 0.08 || MaxError > 0.20 || (ABestFitResult.Valid && AverageError >= ABestFitResult.AverageError)) return;
			...
			TetAngleSpanResult FindMinimalAngleSpan(const CetPath& AChain, double ACenterX, double ACenterY)
			{
				...
				if (AChain.size() < 2) return Result;
				...
				if (Angles.size() < 2) return Result;
				...
				if (SweepAngle < CET_GENERAL_ARC_MIN_SWEEP || SweepAngle > CET_GENERAL_ARC_MAX_SWEEP) return Result;
			...
			bool TryAnalyzeGeneralThickArcFeature(const CetPath& AContour, TetShapeFeature& AFeature)
			{
				...
				if (AreaError > 0.22) return false;
				...
				std::cout << "[SHAPE][ARC][GENERAL_THICK] Index=" << AFeature.OriginalIndex
					<< " Type=" << static_cast<int>(AFeature.ArcType) << " OuterRadius=" << AFeature.ArcRadius
					<< " InnerRadius=" << ChainFit.InnerRadius << " Sweep=" << AFeature.ArcSweepAngle
					<< " Chord=" << AFeature.ArcChordLength << " FitError=" << AFeature.ArcFitError
					<< " BulgeSign=" << AFeature.ArcBulgeSign << std::endl;
				return true;
			}
		}
		...
		void CetShapeAnalyzer::_AnalyzeRectangleFeature(const CetPath& AContour, TetShapeFeature& AFeature)
		{
			...
			if (!CetClusterMathUtils::NearlyEqual(EdgeLengths[0], EdgeLengths[2], LengthTolerance) ||
				!CetClusterMathUtils::NearlyEqual(EdgeLengths[1], EdgeLengths[3], LengthTolerance)) {
				return;
			}
		...
		void CetShapeAnalyzer::_AnalyzeArcFeature(const CetPath& AContour, TetShapeFeature& AFeature)
		{
			...
			if (AFeature.HasHoles || AContour.size() < 5) return;
		...
		void CetShapeAnalyzer::_AnalyzeEllipseFeature(const CetPath& AContour, TetShapeFeature& AFeature)
		{
			...
			if (AFeature.HasHoles || !AFeature.IsConvex || AContour.size() < 8) return;
			if (AFeature.Width <= CET_SHAPE_EPSILON || AFeature.Height <= CET_SHAPE_EPSILON) return;
			...
			if (RadiusX <= CET_SHAPE_EPSILON || RadiusY <= CET_SHAPE_EPSILON) return;
			...
			if (AverageError > 0.12 || MaxError > 0.35) return;
			...
			if (MinorAxis <= CET_SHAPE_EPSILON) return;
			...
			std::cout << "[SHAPE][ELLIPSE] Index=" << AFeature.OriginalIndex << " Major=" << AFeature.EllipseMajorAxis
				<< " Minor=" << AFeature.EllipseMinorAxis << " FitError=" << AFeature.EllipseFitError << std::endl;
		}
		...
		void CetShapeAnalyzer::_NormalizePath(CetPath& APath)
		{
			...
			while (Changed && Clean.size() > 3) {
				...
				for (std::size_t i = 0; i < Count; ++i) {
					...
					const long double Cross = static_cast<long double>(B.X - A.X) * static_cast<long double>(C.Y - B.Y) -
						static_cast<long double>(B.Y - A.Y) * static_cast<long double>(C.X - B.X);
		...
		{
			if (AFeature.HasHoles || AFeature.IsConvex || AContour.size() < 5) return false;
			...
			if (Width >= Height * 1.40) {
				...
				_EvaluateThickArcCandidate(AContour, AFeature, TetThickArcTestInput{
					CenterX, MinY, OuterRadius, true, 1, 0.0,
				...
				_EvaluateThickArcCandidate(AContour, AFeature, TetThickArcTestInput{
					CenterX, MaxY, OuterRadius, true, -1, 0.0,
			...
			if (Height >= Width * 1.40) {
				...
				_EvaluateThickArcCandidate(AContour, AFeature, TetThickArcTestInput{
					MinX, CenterY, OuterRadius, false, 1, CET_CLUSTER_HALF_PI,
				...
				_EvaluateThickArcCandidate(AContour, AFeature, TetThickArcTestInput{
					MaxX, CenterY, OuterRadius, false, -1, CET_CLUSTER_HALF_PI,
			...
			std::cout << "[SHAPE][ARC][THICK] Index=" << AFeature.OriginalIndex << " OuterRadius=" << AFeature.ArcRadius
				<< " InnerRadius=" << BestCandidate.InnerRadius << " Chord=" << AFeature.ArcChordLength
				<< " FitError=" << AFeature.ArcFitError << " BulgeSign=" << AFeature.ArcBulgeSign << std::endl;
		...
		{
			if (AInput.OuterRadius <= CET_SHAPE_EPSILON) return;

			const double InnerRadiusSquared = AInput.OuterRadius * AInput.OuterRadius - 2.0 * AFeature.Area / CET_CLUSTER_PI;
			if (InnerRadiusSquared <= CET_SHAPE_EPSILON || InnerRadiusSquared >= AInput.OuterRadius * AInput.OuterRadius) return;

			const double InnerRadius = std::sqrt(InnerRadiusSquared);
			const double Thickness = AInput.OuterRadius - InnerRadius;
			if (Thickness <= CET_SHAPE_EPSILON || Thickness < AInput.OuterRadius * 0.02 || Thickness > AInput.OuterRadius * 0.80) return;
		...
		bool CetShapeAnalyzer::_AnalyzeSolidArcFeature(const CetPath& AContour, TetShapeFeature& AFeature)
		{
			...
			if (!_FindLongestContourEdge(AContour, ChordStartIndex, ChordEndIndex, MaxEdgeLength)) return false;
			...
			std::cout << "[SHAPE][ARC] Index=" << AFeature.OriginalIndex << " Radius=" << AFeature.ArcRadius
				<< " Chord=" << AFeature.ArcChordLength << " FitError=" << AFeature.ArcFitError
				<< " BulgeSign=" << AFeature.ArcBulgeSign << std::endl;
