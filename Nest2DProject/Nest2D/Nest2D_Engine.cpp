    ...
    namespace NEST2DMANAGERLIB {
        ...
        static bool BuildBoardSubjectContours(const TetNestOptions &AOptions, ClipperLib::Paths &AOutContours)
        {
            ...
            if (AOptions.Board.Enabled && AOptions.Board.Vertices.size() >= 3) {
                CetPath Outer;
                if (!BuildBoardPath(AOptions.Board.Vertices, true, Outer))
                    return false;
                AOutContours.push_back(std::move(Outer));
                for (const auto &HoleVertices : AOptions.Board.Holes) {
                    CetPath Hole;
                    if (!BuildBoardPath(HoleVertices, false, Hole))
                        return false;
            ...
            CetPath Outer;
            if (!BuildBoardPath(Rectangle, true, Outer))
                return false;
            AOutContours.push_back(std::move(Outer));
            return true;
        }
        static bool BuildPlacedReservedContours(const CetTNestItemVector &AItems, int ABinId, double ASpacing, ClipperLib::Paths &AOutContours, libnest2d::Coord AExtraInflation = 0)
        {
            AOutContours.clear();
        ...
        static bool LocalCompactGetBounds(const CetNestItem &AItem, double &AOutMinX, double &AOutMinY, double &AOutMaxX, double &AOutMaxY)
        {
            CetNestItem Item = AItem;
            Item.inflation(0);
            const auto Bounds = Item.boundingBox();
        ...
        static TetLocalCompactEnvelope LocalCompactCalculateEnvelope(const CetTNestItemVector &AItems, int ABinId, const std::vector<bool> *AExcluded = nullptr)
        {
            ...
            for (std::size_t Index = 0; Index < AItems.size(); ++Index) {
                ...
                if (!LocalCompactGetBounds(AItems[Index], MinX, MinY, MaxX, MaxY))
                    continue;
                if (!Result.Valid) {
                    Result.Valid = true;
                    Result.MinX = MinX;
        ...
        static void LocalCompactAppendFreeRegions(const ClipperLib::PolyNode &ANode, std::vector<TetClusterFreeRegion> &AOutRegions)
        {
        ...
        static bool BuildLocalCompactFreeRegions(const CetTNestItemVector &AItems, const TetNestOptions &AOptions, int ABinId, const std::vector<bool> &ATargetMask, std::vector<TetClusterFreeRegion> &AOutRegions)
        {
            AOutRegions.clear();
            ClipperLib::Paths Board;
            if (!BuildBoardSubjectContours(AOptions, Board))
                return false;
            ...
            if (!BuildPlacedReservedContours(FrozenItems, ABinId, AOptions.Spacing, Reserved, HalfSpacing))
                return false;
            ...
            for (const ClipperLib::PolyNode *Node : Tree.Childs)
                if (Node != nullptr)
                    LocalCompactAppendFreeRegions(*Node, AOutRegions);
        ...
        static TetLocalCompactFreeSpaceMetric LocalCompactCalculateFreeSpaceMetric(const CetTNestItemVector &AItems, const TetNestOptions &AOptions, int ABinId)
        {
            ...
            const auto HalfSpacing = static_cast<libnest2d::Coord>(std::ceil(static_cast<double>(NestUtils::ToNestCoord(std::max(0.0, AOptions.Spacing))) * 0.5));
            ClipperLib::Paths Reserved;
            if (!BuildPlacedReservedContours(AItems, ABinId, AOptions.Spacing, Reserved, HalfSpacing))
                return Result;
            ClipperLib::Clipper Difference;
            if (!Difference.AddPaths(Board, ClipperLib::ptSubject, true) || (!Reserved.empty() && !Difference.AddPaths(Reserved, ClipperLib::ptClip, true)))
                return Result;
        ...
        static std::map<int, std::string> LocalCompactBuildSkippedBins(const CetTNestItemVector &AItems, const TetNestOptions &AOptions)
        {
        ...
        static void LocalCompactApplyPose(CetTNestItemVector &AItems, const TetLocalCompactTarget &ATarget, double ARotation, double AAnchorX, double AAnchorY)
        {
        ...
        static TetLocalCompactEnvelope LocalCompactCalculateTargetEnvelope(const CetTNestItemVector &AItems, const TetLocalCompactTarget &ATarget)
        {
            TetLocalCompactEnvelope Result;
        ...
        static TetLocalCompactEnvelope LocalCompactMergeEnvelopes(const TetLocalCompactEnvelope &AFixed, const TetLocalCompactEnvelope &ATarget)
        {
            if (!AFixed.Valid)
                return ATarget;
            if (!ATarget.Valid)
        ...
        static bool LocalCompactTryBuildClusterTarget(const CetTNestItemVector &AItems, const TetMetaItem &AMeta, TetLocalCompactTarget &AOutTarget)
        {
            ...
            for (const TetItemTransform &Transform : AMeta.TransformData) {
                ...
                if (Item.binId() != FirstItem.binId() || LocalCompactAngleDistance(static_cast<double>(Item.rotation()), Rotation + Transform.RelativeRotation) > 1e-8)
                    return false;
        ...
        static bool LocalCompactTryRecoverCurrentClusterTarget(const CetTNestItemVector &AItems, const TetMetaItem &AMeta, TetLocalCompactTarget &AOutTarget)
        {
            AOutTarget = TetLocalCompactTarget{};
            if (!AMeta.IsCluster || AMeta.TransformData.size() < 2)
                return false;
        ...
        static std::vector<TetLocalCompactTarget> BuildLocalCompactTargets(const CetTNestItemVector &AItems, const std::vector<TetMetaItem> *AMetaItems)
        {
            ...
            if (AMetaItems != nullptr)
                for (const TetMetaItem &Meta : *AMetaItems) {
                    ...
                    if (LocalCompactTryBuildClusterTarget(AItems, Meta, Target) || LocalCompactTryRecoverCurrentClusterTarget(AItems, Meta, Target)) {
                        for (const TetItemTransform &Transform : Meta.TransformData)
        ...
        static bool LocalCompactIsStrictImprovement(const TetLocalCompactEnvelope &ACandidate, const TetLocalCompactEnvelope &ABaseline)
        {
        ...
        static bool LocalCompactIsNonWorsening(const TetLocalCompactEnvelope &ACandidate, const TetLocalCompactEnvelope &ABaseline)
        {
            if (!ACandidate.Valid || !ABaseline.Valid)
                return false;
            const double AreaEpsilon = std::max({1.0, std::abs(ACandidate.Area), std::abs(ABaseline.Area)}) * 1e-9;
        ...
        static bool LocalCompactIsFreeSpaceBetter(const TetLocalCompactFreeSpaceMetric &ACandidate, const TetLocalCompactFreeSpaceMetric &ABaseline)
        {
            if (!ACandidate.Valid || !ABaseline.Valid)
                return false;
        ...
        static bool LocalCompactIsCandidateBetter(const TetLocalCompactCandidate &ACandidate, const TetLocalCompactCandidate &ABest)
        {
        ...
        static void EvaluateBoardFreeRegionMetrics(const CetTNestItemVector &AItems, const TetNestOptions &AOptions, TetTNestEvalResult &AInOutResult)
        {
            ...
            for (std::size_t Bin = 0; Bin < AInOutResult.Layers; ++Bin) {
                ClipperLib::Paths Reserved;
                if (!BuildPlacedReservedContours(AItems, static_cast<int>(Bin), AOptions.Spacing, Reserved))
                    return;
                ClipperLib::Clipper Difference;
                if (!Difference.AddPaths(Board, ClipperLib::ptSubject, true) || (!Reserved.empty() && !Difference.AddPaths(Reserved, ClipperLib::ptClip, true)))
                    return;
                ...
                for (const ClipperLib::PolyNode *Node : Tree.Childs)
                    if (Node != nullptr)
                        AccumulateFreeRegions(*Node, Count, TotalArea, LargestArea);
        ...
        static double GetMinimumPassableWidth(const CetTNestItemVector &AItems, const TetNestOptions &AOptions)
        {
            ...
            for (const CetNestItem &SourceItem : AItems) {
                ...
                for (const Radians Rotation : Rotations) {
                    ...
                    const auto Bounds = Item.boundingBox();
                    const double ShortSide = std::min(std::abs(static_cast<double>(Bounds.width())), std::abs(static_cast<double>(Bounds.height())));
                    if (ShortSide > 1.0 && std::isfinite(ShortSide))
                        MinimumWidth = std::min(MinimumWidth, ShortSide);
                }
            }
            return std::isfinite(MinimumWidth) ? MinimumWidth : 0.0;
        }
        static bool BuildInsetBoardContours(const ClipperLib::Paths &ABoard, libnest2d::Coord AInset, ClipperLib::Paths &AOutContours)
        {
            AOutContours = ABoard;
            if (AInset <= 0)
        ...
        static void EvaluatePassableFreeRegionMetrics(const CetTNestItemVector &AItems, const TetNestOptions &AOptions, TetTNestEvalResult &AInOutResult)
        {
            ...
            if (AInOutResult.MinimumPassableWidth <= 0.0)
                return;
            ClipperLib::Paths Board;
            if (!BuildBoardSubjectContours(AOptions, Board))
                return;
            ...
            if (!BuildInsetBoardContours(Board, SideAllowance, AvailableBoard))
                return;
            for (std::size_t Bin = 0; Bin < AInOutResult.Layers; ++Bin) {
                ClipperLib::Paths Reserved;
                if (!BuildPlacedReservedContours(AItems, static_cast<int>(Bin), AOptions.Spacing, Reserved, SideAllowance))
                    return;
                ClipperLib::Clipper Difference;
                if (!Difference.AddPaths(AvailableBoard, ClipperLib::ptSubject, true) || (!Reserved.empty() && !Difference.AddPaths(Reserved, ClipperLib::ptClip, true)))
                    return;
                ClipperLib::PolyTree Tree;
                if (!Difference.Execute(ClipperLib::ctDifference, Tree, ClipperLib::pftNonZero, ClipperLib::pftNonZero))
                    return;
        ...
        {
            if (!ACandidate.HasPassableFreeRegionMetric || !ABaseline.HasPassableFreeRegionMetric)
                return true;
            const double AreaTolerance = std::max({1.0, std::abs(ACandidate.LargestPassableFreeRegionArea), std::abs(ABaseline.LargestPassableFreeRegionArea)}) * 1e-9;
            if (ACandidate.FragmentedPassableFreeArea > ABaseline.FragmentedPassableFreeArea + AreaTolerance)
                return false;
            if (ACandidate.LargestPassableFreeRegionArea + AreaTolerance < ABaseline.LargestPassableFreeRegionArea)
                return false;
            return ACandidate.PassableFreeRegionCount <= ABaseline.PassableFreeRegionCount;
        }
        static bool IsBetterContinuousFreeSpace(const TetTNestEvalResult &ACandidate, const TetTNestEvalResult &ABaseline)
        {
            const bool UsePassableMetrics = ACandidate.HasPassableFreeRegionMetric && ABaseline.HasPassableFreeRegionMetric;
            const bool UseBoardMetrics = ACandidate.HasBoardFreeRegionMetric && ABaseline.HasBoardFreeRegionMetric;
            if (!UsePassableMetrics && !UseBoardMetrics)
                return false;
            const double CandidateArea = UsePassableMetrics ? ACandidate.LargestPassableFreeRegionArea : ACandidate.LargestFreeRegionArea;
            const double BaselineArea = UsePassableMetrics ? ABaseline.LargestPassableFreeRegionArea : ABaseline.LargestFreeRegionArea;
