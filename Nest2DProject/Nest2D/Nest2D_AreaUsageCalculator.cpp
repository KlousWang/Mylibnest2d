...
namespace {
    ClipperLib::Path TransformContour(const std::vector<TetNestPoint> &APoints, const TetNestPolygon &AItem)
    {
        ...
        for (const TetNestPoint &Point : APoints) {
            ...
            Result.emplace_back(static_cast<ClipperLib::cInt>(std::llround(X * CosAngle - Y * SinAngle + TranslationX)), static_cast<ClipperLib::cInt>(std::llround(X * SinAngle + Y * CosAngle + TranslationY)));
        }
        return Result;
    }
...
namespace ET {
    namespace NEST2DMANAGERLIB {
        ...
        double CetAreaUsageCalculator::CalcPolygonArea(const TetNestPolygon &APoly) { return CalcNetArea(APoly.Vertices, APoly.Holes); }
