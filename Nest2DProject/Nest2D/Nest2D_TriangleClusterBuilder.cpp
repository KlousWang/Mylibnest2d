...
namespace ET {
    namespace NEST2DMANAGERLIB {
        ...
        bool CetTriangleClusterBuilder::TryMakeRightTrianglePair(const CetTNestItemVector &AOriginalItems, int AAIndex, int ABIndex, const TetNestOptions &AOptions, TetClusterBuildResult &AResult)
        {
            ...
            if (!RightA || !RightB) {
                std::cout << "[CLUSTER][REJECT] not right triangle: " << AAIndex << " + " << ABIndex << ", A(W,H,Area)=(" << WA << "," << HA << "," << AreaA << ")"
                          << ", B(W,H,Area)=(" << WB << "," << HB << "," << AreaB << ")"
                          << ", A ratio=" << (WA * HA > 0.0 ? AreaA * 2.0 / (WA * HA) : 0.0) << ", B ratio=" << (WB * HB > 0.0 ? AreaB * 2.0 / (WB * HB) : 0.0) << std::endl;
                return false;
            }
            ...
            if (ClusterW > BinW || ClusterH > BinH) {
                std::cout << "[CLUSTER][REJECT] cluster bigger than bin: " << AAIndex << " + " << ABIndex << ", W = " << W << ", H = " << H << ", InternalSpacing = " << InternalSpacing << ", AxisGap = " << AxisGap << ", ClusterW = " << ClusterW << ", ClusterH = " << ClusterH << ", BinW = " << BinW << ", BinH = " << BinH << std::endl;
                return false;
            }
            ...
            std::cout << "[CLUSTER] RightTrianglePair created: " << AAIndex << " + " << ABIndex << ", W = " << W << ", H = " << H << ", InternalSpacing = " << InternalSpacing << ", AxisGap = " << AxisGap << ", ClusterW = " << ClusterW << ", ClusterH = " << ClusterH << ", PackedIndex = " << PackedIndex << std::endl;
            return true;
        }
        void CetTriangleClusterBuilder::BuildCandidates(const CetTNestItemVector &AOriginalItems, const std::vector<TetShapeFeature> &AFeatures, const std::vector<int> &AIndices, const TetNestOptions &AOptions, std::vector<TetClusterCandidate> &AOutCandidates)
        {
            ...
            std::cout << "[TRIANGLE][BUILD CANDIDATES] IndexCount=" << AIndices.size() << ", GroupCount=" << Groups.size() << ", RightRectangleHandled=" << RightRectangleHandledIndices.size() << ", NewCandidateCount=" << AOutCandidates.size() - OldCandidateCount << std::endl;
        }
        ...
        double CetTriangleClusterBuilder::_GetItemWidth(const CetNestItem &AItem) { return static_cast<double>(AItem.boundingBox().width()); }
        double CetTriangleClusterBuilder::_GetItemHeight(const CetNestItem &AItem) { return static_cast<double>(AItem.boundingBox().height()); }
                ...
                while (GroupOffset + 1 < Group.size()) {
                    ...
                    while (TrialCount >= 2) {
                        ...
                        if (ReuseFirstCandidate || _BuildRightTriangleRectangleClusterCandidate(AOriginalItems, AFeatures, ClusterIndices, AOptions, Candidate)) {
                            ...
                            std::cout << "[TRIANGLE][RECTANGLE][CANDIDATE] GroupSize=" << Group.size() << ", Offset=" << GroupOffset << ", UsedCount=" << AOutCandidates.back().OriginalIndices.size() << ", Type=" << AOutCandidates.back().ClusterType << ", Score=" << AOutCandidates.back().Score << std::endl;
        ...
        bool CetTriangleClusterBuilder::_BuildRightTriangleRectangleLayoutCandidate(const TetRightTriangleRectangleLayoutRequest &ARequest, TetClusterCandidate &AOutCandidate)
        {
            ...
            Candidate.ClusterType = AIndices.size() == 2 ? "RightTriangleRectanglePair" : "RightTriangleRectangleGrid_" + std::to_string(AIndices.size()) + "_R" + std::to_string(ACellRows);
        ...
        double CetTriangleClusterBuilder::_CalculateRightTriangleRectangleScore(const TetClusterCandidate &ACandidate, int APairCount, int ACellRows, int ACellCols)
        {
            ...
            const double GridBalance = ACellRows > 0 && ACellCols > 0 ? 1.0 / static_cast<double>(1 + std::abs(ACellRows - ACellCols)) : 0.0;
            const double PerimeterPenalty = (ACandidate.ClusterWidth + ACandidate.ClusterHeight) * 0.000001;
            return 5000.0 + ACandidate.FillRatio * 3000.0 + ACandidate.AreaSavingRatio * 5000.0 + static_cast<double>(ACandidate.OriginalIndices.size()) * 140.0 + static_cast<double>(APairCount) * 180.0 + CompactRatio * 20.0 + GridBalance * 20.0 + ACandidate.Confidence * 100.0 - ACandidate.ProxyWasteRatio * 250.0 - PerimeterPenalty;
        }
        bool CetTriangleClusterBuilder::_AreCongruentTriangles(const TetShapeFeature &AA, const TetShapeFeature &AB)
        {
            ...
            for (int i = 0; i < 3; ++i)
                if (!CetClusterMathUtils::NearlyEqual(AA.TriangleSides[i], AB.TriangleSides[i], 0.03))
                    return false;
            return true;
        }
        void CetTriangleClusterBuilder::_BuildAnyTriangleClusterCandidates(const CetTNestItemVector &AOriginalItems, const std::vector<TetShapeFeature> &AFeatures, const std::vector<int> &AIndices, const TetNestOptions &AOptions, std::vector<TetClusterCandidate> &AOutCandidates)
        {
            ...
            while (GroupOffset + 1 < RemainingIndices.size()) {
                ...
                std::cout << "[TRIANGLE][GENERAL][CANDIDATE] ChildCount=" << BestCandidate.OriginalIndices.size() << ", Type=" << BestCandidate.ClusterType << ", Width=" << BestCandidate.ClusterWidth << ", Height=" << BestCandidate.ClusterHeight << ", Score=" << BestCandidate.Score << std::endl;
        ...
        bool CetTriangleClusterBuilder::_TryBuildAnyTriangleGridCandidate(const CetTNestItemVector &AOriginalItems, const TetNestOptions &AOptions, const std::vector<int> &AIndices, const std::vector<TetClusterCandidate> &APairCandidates, const TetTrianglePairCell &ACell, TetClusterCandidate &AOutCandidate)
        {
            ...
            for (std::size_t LayoutIndex = 0; LayoutIndex < MaxLayoutChecks; ++LayoutIndex) {
                ...
                TetClusterCandidate Candidate;
                Candidate.BuilderName = "TriangleBuilder";
                Candidate.ClusterType = AIndices.size() == 2 ? "AnyTrianglePair" : "AnyTriangleGrid_" + std::to_string(AIndices.size()) + "_R" + std::to_string(Layout.Rows);
                Candidate.OriginalIndices = AIndices;
        ...
        {
            if (ARequest.AIndex < 0 || ARequest.BIndex < 0 || ARequest.AIndex == ARequest.BIndex || ARequest.AIndex >= static_cast<int>(ARequest.OriginalItems.size()) || ARequest.BIndex >= static_cast<int>(ARequest.OriginalItems.size()) || ARequest.AIndex >= static_cast<int>(ARequest.Features.size()) || ARequest.BIndex >= static_cast<int>(ARequest.Features.size())) {
                return false;
            }
        ...
        bool CetTriangleClusterBuilder::_BuildOppositeTrianglePairCandidate(const CetTNestItemVector &AOriginalItems, const std::vector<TetShapeFeature> &AFeatures, int AAIndex, int ABIndex, const TetNestOptions &AOptions, TetClusterCandidate &AOutCandidate)
        {
            ...
            if (!Geometry.GetBounds(PathA, AMinX, AMinY, AMaxX, AMaxY))
                return false;
            if (!Geometry.GetBounds(PathB, BMinX, BMinY, BMaxX, BMaxY))
                return false;
            const double RequiredGap = std::max(0.0, static_cast<double>(NestUtils::ToNestCoord(AOptions.Spacing)));
            const double SafetyGap = RequiredGap > 0.0 ? std::max(CET_CLUSTER_MIN_SAFETY_GAP, RequiredGap * 0.001) : 0.0;
