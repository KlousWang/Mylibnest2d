...
namespace ET {
    namespace NEST2DMANAGERLIB {
        ...
        {
            AOutRegions.clear();
            if (!ACandidate.Valid || ACandidate.ProxyContour.size() < 3 || ACandidate.ProxyArea <= 0.0)
                return false;
            CetPath ProxyContour;
            if (!_NormalizeContourForClipper(ACandidate.ProxyContour, ProxyContour, 0.0))
                return false;
            ClipperLib::Paths ReservedContours;
            if (!_BuildReservedChildContours(AOriginalItems, AOptions, ACandidate, ReservedContours))
                return false;
            ClipperLib::Clipper DifferenceClipper;
            if (!DifferenceClipper.AddPath(ProxyContour, ClipperLib::ptSubject, true))
                return false;
            ...
            for (const ClipperLib::PolyNode *Node : Tree.Childs) {
                if (Node != nullptr && !_AppendFreeRegion(*Node, AOutRegions))
                    return false;
            }
            ...
            if (AOutRegions.size() > CET_CLUSTER_FILL_MAX_FREE_REGIONS)
                AOutRegions.resize(CET_CLUSTER_FILL_MAX_FREE_REGIONS);
            return true;
        }
        bool CetClusterGeometryHelper::IntersectFreeRegionsWithRectangle(const std::vector<TetClusterFreeRegion> &ARegions, double AWidth, double AHeight, std::vector<TetClusterFreeRegion> &AOutRegions) const
        {
            AOutRegions.clear();
            ...
            for (const TetClusterFreeRegion &Region : ARegions) {
                if (!Region.IsClosed || Region.Contour.size() < 3)
                    continue;
                ClipperLib::Clipper Clipper;
                if (!Clipper.AddPath(Region.Contour, ClipperLib::ptSubject, true))
                    continue;
                if (!Region.Holes.empty() && !Clipper.AddPaths(Region.Holes, ClipperLib::ptSubject, true))
                    continue;
                if (!Clipper.AddPath(Rectangle, ClipperLib::ptClip, true))
                    continue;
                ClipperLib::PolyTree Tree;
            ...
            std::stable_sort(AOutRegions.begin(), AOutRegions.end(), [](const TetClusterFreeRegion &A, const TetClusterFreeRegion &B) { return std::abs(A.Area - B.Area) > CET_CLUSTER_GEOMETRY_AREA_TOLERANCE ? A.Area > B.Area : A.MinY < B.MinY; });
            if (AOutRegions.size() > CET_CLUSTER_FILL_MAX_FREE_REGIONS)
                AOutRegions.resize(CET_CLUSTER_FILL_MAX_FREE_REGIONS);
        ...
        {
            if (!AFreeRegion.IsClosed || !IsContourFullyContained(AContour, AFreeRegion.Contour, AAreaTolerance))
                return false;
            for (const CetPath &Hole : AFreeRegion.Holes) {
                ClipperLib::Clipper IntersectionClipper;
                if (!IntersectionClipper.AddPath(AContour, ClipperLib::ptSubject, true) || !IntersectionClipper.AddPath(Hole, ClipperLib::ptClip, true))
                    return false;
                ClipperLib::Paths Intersections;
                if (!IntersectionClipper.Execute(ClipperLib::ctIntersection, Intersections, ClipperLib::pftNonZero, ClipperLib::pftNonZero))
                    return false;
                if (_CalculateUnionArea(Intersections) > AAreaTolerance)
                    return false;
            }
            return true;
        }
        ...
        bool CetClusterGeometryHelper::_AppendFreeRegion(const ClipperLib::PolyNode &ANode, std::vector<TetClusterFreeRegion> &AOutRegions) const
        {
            if (!ANode.IsHole() && ANode.Contour.size() >= 3) {
                ...
                if (!std::isfinite(Region.Area) || Region.Area <= 0.0 || !GetBounds(Region.Contour, Region.MinX, Region.MinY, Region.MaxX, Region.MaxY))
                    return false;
            ...
            for (const ClipperLib::PolyNode *Child : ANode.Childs)
                if (Child != nullptr && !_AppendFreeRegion(*Child, AOutRegions))
                    return false;
            return true;
        }
        ...
        {
            if (ACandidate.ClusterWidth <= 0.0 || ACandidate.ClusterHeight <= 0.0)
                return false;
        ...
        bool CetClusterGeometryHelper::_HaveRequiredSpacing(const CetTNestItemVector &AOriginalItems, const TetNestOptions &AOptions, const TetItemTransform &AFirstTransform, const TetItemTransform &ASecondTransform) const
        {
            ...
            if (SpacingCoord > 0.0) {
                ...
                return !CetNestItem::intersects(InflatedFirstItem, InflatedSecondItem) || CetNestItem::touches(InflatedFirstItem, InflatedSecondItem);
            }
            return !CetNestItem::intersects(FirstItem, SecondItem) || CetNestItem::touches(FirstItem, SecondItem);
        }
        ...
        {
            if (!_ValidateIndexAndTransforms(AOriginalItems, ACandidate))
                return false;
            if (ACandidate.ProxyMode == MetClusterProxyMode::Unknown)
                return false;
            if (ACandidate.ProxyContour.size() < 3 || ACandidate.ProxyArea <= 0.0 || !std::isfinite(ACandidate.ProxyArea))
                return false;
            if (!std::isfinite(ACandidate.ProxyWasteRatio) || ACandidate.ProxyWasteRatio < 0.0 || ACandidate.ProxyWasteRatio > 1.0)
                return false;
            if (!_ValidateChildContainment(AOriginalItems, ACandidate))
                return false;
            if (!_ValidateChildSpacing(AOriginalItems, AOptions, ACandidate, true))
                return false;
            return true;
        }
        ...
        bool CetClusterGeometryHelper::FinalizeCandidate(const CetTNestItemVector &AOriginalItems, const TetNestOptions &AOptions, TetClusterCandidate &ACandidate) const { return FinalizeCandidate(AOriginalItems, AOptions, ACandidate, false); }
        ...
        bool CetClusterGeometryHelper::_CalculateCandidateGeometry(const CetTNestItemVector &AOriginalItems, const TetClusterCandidate &ACandidate, TetCandidateGeometryStats &AOutStats) const
        {
            for (const TetItemTransform &Transform : ACandidate.Transforms) {
                ...
                if (!GetBounds(Child, ChildMinX, ChildMinY, ChildMaxX, ChildMaxY))
                    return false;
                ...
                if (!std::isfinite(OriginalArea) || !std::isfinite(ChildBoxArea) || OriginalArea <= 0.0 || ChildBoxArea <= 0.0)
                    return false;
        ...
        bool CetClusterGeometryHelper::_NormalizeCandidateProxy(TetClusterCandidate &ACandidate, const TetCandidateGeometryStats &AStats) const
        {
            ...
            if (!std::isfinite(ACandidate.ProxyArea) || ACandidate.ProxyArea <= 0.0)
                return false;
            return true;
        }
        ...
        bool CetClusterGeometryHelper::FinalizeCandidate(const CetTNestItemVector &AOriginalItems, const TetNestOptions &AOptions, TetClusterCandidate &ACandidate, bool AForceRectangleProxy) const
        {
            ...
            if (!_CalculateCandidateGeometry(AOriginalItems, ACandidate, GeometryStats))
                return false;
            ...
            if (!_BuildTransformedChildContours(AOriginalItems, ACandidate, TransformedChildContours))
                return false;
            ACandidate.OccupiedArea = _CalculateUnionArea(TransformedChildContours);
            if (!std::isfinite(ACandidate.OccupiedArea) || ACandidate.OccupiedArea <= 0.0)
                return false;
            ...
            if (!_NormalizeCandidateProxy(ACandidate, GeometryStats))
                return false;
            if (!_CalculateCandidateMetrics(ACandidate, AOptions))
                return false;
            ACandidate.ProxyContourNormalized = true;
            if (!_FitsBoardBounds(ACandidate, AOptions))
                return false;
            if (!ValidateCandidateGeometry(AOriginalItems, AOptions, ACandidate))
                return false;
