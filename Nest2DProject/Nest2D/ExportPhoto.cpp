...
namespace {
    std::string MakeUtilizationSvgText(const TetBoardUsageResult &AUsage, double ASvgWidth, double ASvgHeight)
    {
        ...
        ss << "<text x=\"" << Margin << "\" y=\"" << (Margin + FontSize) << "\" font-family=\"Arial, Helvetica, sans-serif\""
           << " font-size=\"" << FontSize << "\" fill=\"#111111\""
           << " fill-opacity=\"0.85\">UsagePercent: " << Percent << "%"
           << " PureUsagePercnt: " << PureUsagePercnt << "%</text>\n";
        return ss.str();
    }
    std::vector<libnest2d::Item> BuildSvgItemsForBin(const std::vector<TetNestPolygon> &AItems, int ABin)
    {
        ...
        for (const auto &Item : AItems) {
            ...
            Path Outer;
            for (const auto &PointData : Item.Vertices) {
                Outer.push_back(Point(NestUtils::ToNestCoord(PointData.X), NestUtils::ToNestCoord(PointData.Y)));
            }
            if (Outer.size() < 3) {
                continue;
            }
            ...
            for (const auto &HoleData : Item.Holes) {
                ...
                Path Inner;
                for (const auto &PointData : HoleData) {
                    Inner.push_back(Point(NestUtils::ToNestCoord(PointData.X), NestUtils::ToNestCoord(PointData.Y)));
                }
                if (ClipperLib::Orientation(Inner)) {
    ...
    namespace NEST2DMANAGERLIB {
        ...
        int CetExportPhoto::ExportSvg(const std::vector<TetNestPolygon> &AItems, const TetNestOptions &AOptions, int AUsedBins)
        {
            ...
            for (int currentBin = 0; currentBin < AUsedBins; ++currentBin) {
                ...
                if (AUsedBins > 1)
                    finalPath = basePath + "_" + std::to_string(currentBin);
                else
                    finalPath = basePath;
                svgw.save(finalPath);
        ...
        int CetExportPhoto::ExportSvgItems(CetTNestItemVector &ANestItems, const TetNestOptions &AOptions, int AUsedBins)
        {
            ...
            if (ANestItems.empty() || AUsedBins <= 0)
                return NEST2D_ERR_EXPORT_EMPTY_ITEMS;
            if (AOptions.SvgPath.empty())
                return NEST2D_ERR_EXPORT_NO_PATH;
